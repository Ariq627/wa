...
const {apiNoBg,} = JSON.parse(fs.readFileSync('./config.json'))
/********** END OF UTILS **********/
...
const simi = JSON.parse(fs.readFileSync('./database/bot/simi.json')) 
/********** END OF DATABASES **********/
    ...
    try {
        ...
        // Simi-simi function
		    if ((!isCmd && isGroupMsg && isSimi) && message.type === 'chat') {
			    axios.get(`http://videfikri.com/api/simsimi/?teks=${encodeURIComponent(message.body)}`)
			    .then((res) => {
				bocchi.reply(from, `${res.data.jawaban}`, id)
			})
			.catch((err) => {
        ...
        switch (command) {
            ...
            case 'ytmp3':
                    ...
                    downloader.ytdl(q)
                        .then(async({ result }) => {
                            if (Number(result.size.split(' MB')[0]) >= 20.00) return bocchi.sendFileFromUrl(from, result.imgUrl, `${result.title}.jpg`, `Judul: ${result.title}\nSize: *${result.size}*\n\nGagal, Maksimal video size adalah *20MB*!`, id)
            ...
            case 'ytmp4':
                    ...
                    downloader.ytdl(q)
                        .then(async({ result }) => {
                            if (Number(result.size.split(' MB')[0]) >= 30.00) return bocchi.sendFileFromUrl(from, result.imgUrl, `${result.title}.jpg`, `Judul: ${result.title}\nSize: *${result.size}*\n\nGagal, Maksimal video size adalah *30MB*!`, id)
                                await bocchi.sendFileFromUrl(from, result.imgUrl, `${result.title}.jpg`, ind.ytFound(result), id)
            ...
            case 'tiktok': 
                        ...
                        case 'tiktoknowm': 
                            ...
                            downloader.tik2(url)
                                .then(async ({ result })=> {
                                    await bocchi.sendFileFromUrl(from, result.mp4direct, 'tiktok.mp4', '', id)
                                })
                                .catch(async (err) => {
                                    console.log(err)
                                    await bocchi.reply(from, 'Error!', id)
            ...
            case 'simisimi':
                    if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                    if (!isGroupMsg) return bocchi.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
                    bocchi.reply(from, `Untuk mengaktifkan simi-simi pada Group Chat\n\nPenggunaan\n${prefix}simi on --mengaktifkan\n${prefix}simi off --nonaktifkan\n`, id)
                    break
                case 'simi':
                    if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                    if (!isGroupMsg) return bocchi.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
                    if (args.length !== 1) return bocchi.reply(from, `Untuk mengaktifkan simi-simi pada Group Chat\n\nPenggunaan\n${prefix}simi on --mengaktifkan\n${prefix}simi off --nonaktifkan\n`, id)
                    if (args[0] == 'on') {
                        simi.push(chatId)
                        fs.writeFileSync('./database/bot/simi.json', JSON.stringify(simi))
                        bocchi.reply(from, 'Mengaktifkan bot simi-simi!', id)
                    } else if (args[0] == 'off') {
                            let inxx = simi.indexOf(chatId)
                            simi.splice(inxx, 1)
                            fs.writeFileSync('./database/bot/simi.json', JSON.stringify(simi))
                            bocchi.reply(from, 'Menonaktifkan bot simi-simi!', id)
                        } else {
                            bocchi.reply(from, `Untuk mengaktifkan simi-simi pada Group Chat\n\nPenggunaan\n${prefix}simi on --mengaktifkan\n${prefix}simi off --nonaktifkan\n`, id)
                        }
                        break 
                    ...
                    misc.its(q)
                        .then(async({ result }) => {
                            for (let i = 0; i < result.story.itemlist.length; i++) {
                                ...
                                await bocchi.sendFileFromUrl(from, urlDownload, '', 'By: ASP', id)
                                console.log('Success sending IG Story!')
                            }
                        })
                        .catch(async(err) => {
                            console.error(err)
                            await bocchi.reply(from, 'Maaf username tidak ditemukan/private! silahkan coba kembali', id)
                        })
                    break
            case 'kbbi':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
            ...
            case 'xvids':
                        ...
                        .then(async ({ result }) => {
                            for (let i = 0; i < result.length; i++){
                            const { actors, duration, image, title, url } = result[i]
                                await bocchi.sendFileFromUrl(from, image, `${actors}.jpg`, ind.xvids(actors, duration, title, url))
                                }
            ...
            case 'xvid':
                    ...
                    nsfw.xvid(q)
                    .then(async({ mp4 }) => {
                        if (Number(mp4.split(' MB')[0]) >= 20.00) return bocchi.sendFileFromUrl(from, mp4, `Gagal, Maksimal video size adalah *20MB*!`, id)
            ...
            case 'asupan':
                ...
                fun.asupan()
                    .then(async (body) => {
                        ...
                        await bocchi.sendFileFromUrl(from, `http://sansekai.my.id/ptl_repost/${asupanx}`, 'asupan.mp4', '', id)
            ...
            case 'chika'://Piyobot
                ...
                fun.chika()
                    .then(async (body) => {
                        ...
                        await bocchi.sendFileFromUrl(from, `https://piyobot.000webhostapp.com//${chikax}.mp4`, 'asupan.mp4', '', id)
                        console.log('Success sending video!')
                    })
                    .catch(async (err) => {
                        console.error(err)
                    ...
                    if ((isMedia && isImage || isQuotedImage) && args.length === 0) {
                        ...
                        await bocchi.sendImageAsSticker(from, imageBase64)
                            ...
                            if (isMedia && isImage || isQuotedImage) {
                                ...
                                await bocchi.sendImageAsSticker(from, `data:${mimetype};base64,${result.base64img}`)
                                 .then(async() => {
                                await bocchi.sendText(from, ind.ok())
                                console.log(`Sticker processed for ${processTime(t, moment())} seconds`)
                            })
                            ...
                            if (!isUrl(url)) { await bocchi.reply(from, ind.wrongFormat(), id) }
                            bocchi.sendStickerfromUrl(from, url).then((r) => (!r && r !== undefined)
                                ? bocchi.sendText(from, 'Maaf, link yang kamu kirim tidak memuat gambar.')
                                : bocchi.reply(from, 'Here\'s your sticker')).then(() => console.log(`Sticker Processed for ${processTime(t, moment())} Second`))
                        }else {
                            await bocchi.reply(from, `Tidak ada gambar! Untuk menggunakan ${prefix}sticker\n\n\nKirim gambar dengan caption\n${prefix}sticker <biasa>\n${prefix}sticker nobg <tanpa background>\n\natau Kirim pesan dengan\n${prefix}sticker <link_gambar>`, id)
                        }
                        break
            case 'stickergif':
            case 'stikergif':
