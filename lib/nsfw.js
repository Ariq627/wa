 ...
 * @returns {Object}
 */
const xvid = (url) => new Promise((resolve, reject) => {
    console.log(`Get Xvideos from ${url}`)
    fetchJson(`https://mnazria.herokuapp.com/api/porndownloadxvideos?url=${url}`)
        .then((result) => resolve(result))
        .catch((err) => reject(err))
})
const xvids = (text) => new Promise((resolve, reject) => {
    console.log(`Get series from ${text}`)
    fetchJson(`https://mnazria.herokuapp.com/api/porn?search=${text}`)
        .then((result) => resolve(result))
        .catch((err) => reject(err))
})
